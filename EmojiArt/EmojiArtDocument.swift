...
class EmojiArtDocument: ObservableObject
{
    ...
    func reset_background_location(x: Float, y: Float) {
        emojiArt.reset_background_location(x: x, y: y)
    }
    
    func reset_background_mag(mag: Double) {
    ...
    func there_is_an_emoji_selected() -> Bool {
    ...
    func select_emoji(emoji: EmojiArtModel.Emoji) -> Bool {
        ...
        if let index = emojiArt.emojis.index(matching: emoji) {
            ...
            if !emojiArt.emojis[index].is_selected {
                ...
                there_are_emoji_selected = there_is_an_emoji_selected()
            }
